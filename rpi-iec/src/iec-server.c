...
void getValuesFromDatabase()
{
	...
	for (i = 0; i < 4; i++)
	{
		sprintf(query, "SELECT id FROM %s WHERE iec_name = '%s'", idTable,
				names[i]);
		if (!mysql_query(my_con, query))
		{
			...
			sprintf(query,
					"SELECT Value, Time, Validness FROM %s WHERE id = %d",
					valueTable, atoi(row_temp[0]));
			if (!mysql_query(my_con, query))
			{
...
void updateValues()
{
	...
	for (i = 0; i < 4; i++)
	{
		IedServer_updateTimestampAttributeValue(iedServer, objects[i],
				&values[i].ts);
		IedServer_updateFloatAttributeValue(iedServer, values[i].p_attr,
				values[i].value);
	}
		...
		MmsValue* value, bool test)
{
	...
	for (i = 0; i < 4; i++)
	{
		...
		{
			IedServer_updateUTCTimeAttributeValue(iedServer, objects[i],
					timeStamp);
...
int main(int argc, char** argv)
{
	...
	values[1].p_ts = &iedModel_VaconFreqConverter_DSFC1_AnIn1_t;
	values[2].p_ts = &iedModel_VaconFreqConverter_DSFC1_AnIn2_t;
	values[3].p_ts = &iedModel_VaconFreqConverter_DSFC1_AnIn3_t;
